...
def add_donation():
    ...
    print(f"Thank you, {name}, for donating ₱{amount:.2f} on {donation_date}!\n")
...
def edit_donation():
    ...
    print("\n--- Donor List ---")
    all_entries = [(donor, entry) for donor, entries in donors.items() for entry in entries]
    for i, (name, entry) in enumerate(all_entries, 1):
        print(f"{i}. {name} - ₱{entry['amount']:.2f} | {entry['date']}")
